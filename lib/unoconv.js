...
module.exports = function (reporter, definition) {
  ...
  reporter.afterRenderListeners.add('unoconv', async (req, res) => {
    ...
    const id = uuid()
    const sourcePath = path.join(reporter.options.tempAutoCleanupDirectory, `${id}.${res.meta.fileExtension}`)
    await writeFile(sourcePath, res.content)
    ...
    res.content = await readFile(path.join(reporter.options.tempAutoCleanupDirectory, `${id}.${req.template.unoconv.format}`))
